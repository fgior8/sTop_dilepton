...
void SignalPlots::Fill(UInt_t numberVertices, Float_t MET, Float_t MET_phi, Float_t HT, std::vector<Lepton>& leptons, std::vector<Jet>& jets, Float_t weight, Int_t channel, Int_t cut) {
  ...
  if (cut>1) {
    ...
    h_Ptllb->Fill(getPtllb(leptons[0].lorentzVec(), leptons[1].lorentzVec(), MET, MET_phi).Pt(), weight);
    h_Meff->Fill(getMeff(jets, leptons, MET, MET_phi),weight);
    h_dPhill->Fill(TMath::Abs(getdPhill(leptons[0].lorentzVec(), leptons[1].lorentzVec()))/pi,weight);
    h_dPhiLepMet->Fill(TMath::Abs(getdPhiLepMet(leptons[0].lorentzVec(), MET, MET_phi))/pi,weight);
    h_dPhiLepJet->Fill(TMath::Abs(getdPhiLepJet(leptons[0].lorentzVec(), jets[0].lorentzVec()))/pi,weight);
    h_dPhiJetMet->Fill(TMath::Abs(getdPhiJetMet(jets, MET, MET_phi))/pi,weight);
    h_dPhiPtllbMet->Fill(TMath::Abs(getdPhiPtllbMet(getPtllb(leptons[0].lorentzVec(), leptons[1].lorentzVec(), MET, MET_phi), MET, MET_phi))/pi,weight);
  }
