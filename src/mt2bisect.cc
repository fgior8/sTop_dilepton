...
where array pa[0..2], pb[0..2], pmiss[0..2] contains (mass,px,py) 
	...
	3. Use mt2bisect::get_mt2() to obtain the value of mt2:
...
double mt2bisect::get_mt2(){
  if (!momenta_set)
    {
      cout << "mt2bisect::get_mt2() ==> Please set momenta first!" << endl;
      return 0;
    }
...
void mt2bisect::set_momenta(double* pa0, double* pb0, double* pmiss0){
...
void mt2bisect::set_mn(double mn0){
...
void mt2bisect::print(){
  cout << " mt2bisect::print() ==> pax = " << pax*scale << ";   pay = " << pay*scale << ";   ma = " << ma*scale <<";"<< endl;
  cout << " mt2bisect::print() ==> pbx = " << pbx*scale << ";   pby = " << pby*scale << ";   mb = " << mb*scale <<";"<< endl;
  cout << " mt2bisect::print() ==> pmissx = " << pmissx*scale << ";   pmissy = " << pmissy*scale <<";"<< endl;
  cout << " mt2bisect::print() ==> mn = " << mn_unscale<<";" << endl;
}

//special case, the visible particle is massless
void mt2bisect::mt2_massless(){
  ...
  if (nsols_high == nsols_low)
      ...
      if(foundhigh==0) 
	{
	  ...
	  if(verbose > 0) cout << "mt2bisect::mt2_massless() ==> Deltasq_high not found at event " << nevt <<endl;
  ...
  if(nsols_high == nsols_low)
    { 
      if(verbose > 0) cout << "mt2bisect::mt2_massless() ==> error: nsols_low=nsols_high=" << nsols_high << endl;
      if(verbose > 0) cout << "mt2bisect::mt2_massless() ==> Deltasq_high=" << Deltasq_high << endl;
      if(verbose > 0) cout << "mt2bisect::mt2_massless() ==> Deltasq_low= "<< Deltasq_low << endl;
...
int mt2bisect::nsols_massless(double Dsq){
  double delta;
  delta = Dsq/(2*Easq);
  d1    = d11*delta;
  e1    = e11*delta;
...
void mt2bisect::mt2_bisect(){
  ...
  if( nsols(Deltasq_low) > 0 )
    {
      //cout << "mt2bisect::mt2_bisect() ==> nsolutions(Deltasq_low) > 0"<<endl;
      mt2_b = (double) sqrt(mnsq+Deltasq0);
      return;
    }
  ...
  if(nsols_high == nsols_low || nsols_high == 4)
    {
      ...
      if(foundhigh == 0) 
	{
	  if(verbose > 0) cout << "mt2bisect::mt2_bisect() ==> Deltasq_high not found at event " << nevt << endl;
...
int mt2bisect::find_high(double & Deltasq_high){
  double x0,y0;
  x0 = (c1*d1-b1*e1)/(b1*b1-a1*c1);
  y0 = (a1*e1-b1*d1)/(b1*b1-a1*c1);
...
int mt2bisect::scan_high(double & Deltasq_high){
  int foundhigh = 0 ;
  int nsols_high;
  ...
  tempmass = mn + ma;
  maxmass  = sqrt(mnsq + Deltasq_high);
  if (nevt == 32334) cout << "mt2bisect::scan_high() ==> Deltasq_high = " << Deltasq_high << endl; // ???
...
int mt2bisect::nsols(double Dsq){
...
//inline
int mt2bisect::signchange_n(long double t1, long double t2, long double t3, long double t4, long double t5){
  int nsc;
  nsc=0;
  if(t1*t2>0) nsc++;
  if(t2*t3>0) nsc++;
...
//inline
int mt2bisect::signchange_p(long double t1, long double t2, long double t3, long double t4, long double t5){
  int nsc;
  nsc=0;
  if(t1*t2<0) nsc++;
  if(t2*t3<0) nsc++;
